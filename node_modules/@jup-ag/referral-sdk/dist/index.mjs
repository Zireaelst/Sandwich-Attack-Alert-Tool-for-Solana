...
var RPC_URL = process.env.RPC_URL || process.env.NEXT_PUBLIC_RPC_URL || clusterApiUrl("mainnet-beta");
...
var ReferralProvider = class {
  ...
  getProjectAuthorityPubKey(project) {
    let [projectAuthority] = PublicKey3.findProgramAddressSync(
      ...
      const [tokenAccounts, token2022Accounts] = yield Promise.all(
        [TOKEN_PROGRAM_ID2, TOKEN_2022_PROGRAM_ID].map((programId) => __async(this, null, function* () {
          ...
          return Array.from(possibleTokenAccountSet).reduce((acc, address) => {
            ...
            if (tokenAccount) {
              acc.push({ pubkey: new PublicKey3(address), account: tokenAccount });
            }
            return acc;
          }, new Array());
        }))
    ...
    return __async(this, arguments, function* (referralAccountAddress, strategy = {
      ...
      const tokens = yield (() => __async(this, null, function* () {
        if (strategy.type === "top-tokens") {
          ...
          const tokens2 = (yield (yield fetch("https://tokens.jup.ag/tokens?tags=verified,lst")).json()).map(({ address }) => address);
          return tokens2;
        } else {
      ...
      const tokensWithOwner = yield chunkedGetMultipleAccountInfos(
        this.connection,
        tokens.map((topToken) => new PublicKey3(topToken))
      );
  ...
  getReferralTokenAccountsWithStrategyV2(_0) {
    return __async(this, arguments, function* (referralAccountAddress, strategy = {
      ...
      const referralPubkey = new PublicKey3(referralAccountAddress);
      const [tokenAccounts, token2022Accounts] = yield Promise.all(
        [TOKEN_PROGRAM_ID2, TOKEN_2022_PROGRAM_ID].map((programId) => __async(this, null, function* () {
  ...
  claim(_0) {
    return __async(this, arguments, function* ({
      ...
      const addressLookupTable = yield this.connection.getAddressLookupTable(
        new PublicKey3("GBzQG2iFrPwXjGtCnwNt9S5eHd8xAR8jUMt3QDJpnjud")
      );
      ...
      instructions.unshift(
        ComputeBudgetProgram2.setComputeUnitPrice({ microLamports })
      );
      if (units) {
        instructions.unshift(ComputeBudgetProgram2.setComputeUnitLimit({ units }));
      }
        ...
        [tokenAccounts, token2022Accounts].map((accounts, idx) => __async(this, null, function* () {
          ...
          const claimParams = yield Promise.all(
            tokensWithAmount.map((token) => __async(this, null, function* () {
              ...
              const projectAdminTokenAccount = getAssociatedTokenAddressSync2(
                token.account.mint,
                project.admin,
                true,
                tokenProgramId
          ...
          for (const {
            ...
            if (chunk2 === 4) {
              ...
              if (units) {
                instructions.unshift(
                  ComputeBudgetProgram2.setComputeUnitLimit({ units })
                );
              }
              const messageV0 = new TransactionMessage2({
                payerKey: payerPubKey,
              ...
              chunk2 = 0;
              instructions = [];
              txs.push(new VersionedTransaction2(messageV0));
            }
          }
          return txs;
        }))
